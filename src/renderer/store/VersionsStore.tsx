...
import { showToast } from './SettingsStore';
    ...
    try {
      graph.toPNG((dataUri: string) => {
        if (dataUri) {
          ...
          console.error("Failed to generate PNG data URI.");
...
// Add latest version
export const addLatestVersion = createAsyncThunk<IVersion, { graph: Graph; modelId: string; x: number, y: number; height: number; width: number }, { state: RootState }>(
  'versions/addLatestVersion',
  async ({ modelId, graph, x, y, height, width }: AddLatestVersionArgs, { rejectWithValue }) => {
    try {
