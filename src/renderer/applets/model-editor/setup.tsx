...
const create = (container: HTMLDivElement, gridVisible: 'none' | 'dot' | 'mesh') => new Graph({
  container: container,
  grid: {
    visible: gridVisible === 'none' ? false : true,
  ...
  new History({
    beforeAddCommand: (event: string, args: any) => {
      ...
      if (args && args.current && args.current.items) {
        const items = args.current.items
        for (const item of items) {
          if (item === 'edge-vertices' || 'edge-source-handle' || 'edge-target-handle') {
            console.log("FALSE")
      ...
      if (args && args.previous && args.previous.items) {
        const items = args.previous.items
        for (const item of items) {
          if (item === 'edge-vertices' || 'edge-source-handle' || 'edge-target-handle') {
            console.log("FALSE")
            keepItemInHistory = false;
