...
const ProductsModal: React.FC = () => {
    ...
    if (productsCurrentProduct) {
      const updatedResponsibles =
        productsCurrentProduct.responsibles?.length === 1
          ? [{ id: '', firstName: '', lastName: '', role: '' }]
          : productsCurrentProduct.responsibles?.filter(
              (_, i) => i !== index,
            ) || [];
      dispatch(
        setProductsCurrentProduct({
  ...
  const modalHeader = productsIsCloning
    ? 'Clone Product'
    : productsIsEditing
      ? 'Edit Product'
      : 'Add Product';
  const submitButtonText = productsIsCloning
    ? 'Clone'
    : productsIsEditing
      ? 'Edit'
      : 'Add';
            ...
            {productsCurrentProduct?.responsibles?.map((responsible, index) => (
              <Form.Group width="equal" key={index}>
                ...
                <Button
                  ...
                  circular
                  icon="times"
                  onClick={(e) => removeResponsible(index, e)}
                />
              </Form.Group>
            ))}
          </div>
