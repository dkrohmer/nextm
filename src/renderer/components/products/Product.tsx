...
import { Breadcrumb, Container, Dimmer, Grid, Icon, Label, Loader, Message, Segment } from 'semantic-ui-react';
  ...
  return (
    <Container style={{ marginTop: '5em' }}>
      <Breadcrumb>
        ...
        <Breadcrumb.Section active>{product ? product.name : 'Loading...'}</Breadcrumb.Section>
      </Breadcrumb>

      <Segment basic>
        <Segment style={{ minHeight: '150px' }}>
          <Dimmer active={productIsLoading} inverted={true}>
                  ...
                  <Grid.Column width={12}>
                    {product.responsibles && product.responsibles.length > 0 
                      ? product.responsibles.map((resp, _index) => (
                          <Label key={resp.id} size='tiny' style={{ margin: '2px' }}>
                            <Icon name='user' />
