...
const ModelThumbnail: React.FC<ModelThumbnailProps> = ({ modelId }) => {
  ...
  const thumbnail = useSelector(
    (state: RootState) => state.versions.latestVersionThumbnails[modelId],
  );
  const isLoading = useSelector(
    (state: RootState) =>
      state.versions.latestVersionThumbnailsIsLoading[modelId],
  );
  const error = useSelector(
    (state: RootState) => state.versions.latestVersionThumbnailsError[modelId],
  );
   ...
   * tsx
   */
  return <Image src={handleThumbnail()} className="model-thumbnail" />;
};
