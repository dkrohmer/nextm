...
describe('Products Thunks with Redux Store', () => {
  ...
  it('dispatches fetchProducts and updates the store on success', async () => {
    const mockProducts = { products: [{ id: '1', name: 'Product 1' }], productsCount: 1 };
    ...
    await store.dispatch(fetchProducts({ limit: 10, offset: 0, sort: 'asc', sortby: 'name' }));
  ...
  it('dispatches fetchProduct and updates the store on success', async () => {
    ...
    await store.dispatch(fetchProduct({ productId: '1', isEagerLoading: true }));
  ...
  it('dispatches addOrUpdateProduct and updates the store for an update', async () => {
    ...
    expect(state.products.products.find((product) => product.id === '1')).toBeDefined();
  ...
  it('dispatches deleteIncrement and updates the store on success', async () => {
    ...
    await store.dispatch(deleteProduct({ productId: '1', limit: 10, offset: 0, sort: 'asc', sortby: 'name' }));

    const state = store.getState().products;
    expect(window.electron.deleteProduct).toHaveBeenCalledWith({ productId: '1' });
  });

  it('handles fetchProducts failure correctly', async () => {
    window.electron = {
      ...windowElectron,
      getAllProducts: jest.fn().mockRejectedValue(new Error('Failed to load products.')),
    };
  ...
  it('handles fetchProduct failure correctly', async () => {
    window.electron.getProductById = jest.fn().mockRejectedValue(new Error('Failed to load product.'));
  ...
  it('handles addOrUpdateProduct failure correctly', async () => {
    ...
    window.electron.updateProduct = jest.fn().mockRejectedValue(new Error('Failed to add or update product.'));
  ...
  it('handles deleteProduct failure correctly', async () => {
    window.electron.deleteProduct = jest.fn().mockRejectedValue(new Error('Failed to delete product.'));
