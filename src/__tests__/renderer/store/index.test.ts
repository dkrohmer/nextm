...
describe('Redux Store', () => {
  it('should initialize the store with the correct reducers', () => {
    ...
    expect(state.products).toEqual(productsReducer(undefined, { type: '@@INIT' }));
    expect(state.increments).toEqual(incrementsReducer(undefined, { type: '@@INIT' }));
    expect(state.models).toEqual(modelsReducer(undefined, { type: '@@INIT' }));
    expect(state.versions).toEqual(versionsReducer(undefined, { type: '@@INIT' }));
    expect(state.modelEditor).toEqual(modelEditorReducer(undefined, { type: '@@INIT' }));
