...
export class ModelService {
  ...
  async createModel(data: any): Promise<Model> {
    ...
    const increment = await this.incrementRepository.getIncrementById(incrementId, false);
    if (!increment) {
      throw new Error('Product not found');
    }

    let model: Model = buildModelEntity(data)
  ...
  async getAllModels(sortBy: string, sort: 'asc' | 'desc', incrementId?: string): Promise<{ models: Model[], modelsCount: number }> {
    const [models, modelsCount] = await this.modelRepository.getAllModels(sortBy, sort, incrementId);
