...
export class VersionService {
  ...
  async createVersion(data: any): Promise<Version> {
    ...
    const versionCount = await this.versionRepository.countVersionsByModelId(modelId);
  ...
  async getAllVersions(sortBy: string, sort: 'asc' | 'desc', modelId?: string): Promise<{ versions: Version[], versionsCount: number }> {
    const [versions, versionsCount] = await this.versionRepository.getAllVersions(sortBy, sort, modelId);
  ...
  async getLatestVersionByModelId(modelId: string): Promise<Version | null> {
    const version = await this.versionRepository.getLatestVersionByModelId(modelId);
