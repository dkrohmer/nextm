...
export class IncrementService {
  ...
  async createIncrement(data: Increment): Promise<Increment> {
    ...
    const product = await this.productRepository.getProductById(productId, false);
    if (!product) {
      throw new Error('Product not found');
    }

    // Find the latest version number for the given modelId
    const latestIncrement = await this.incrementRepository.getLatestIncrement(productId)
    // Set the new version number
    const newIncrementIndex = latestIncrement ? latestIncrement.incrementIndex + 1 : 0;
    // increment.incrementIndex = newIncrementIndex

    let increment: Increment = buildIncrementEntity({...data, incrementIndex: null}, newIncrementIndex)
    increment.productId = productId

    const newIncrement = await this.incrementRepository.createIncrement(increment)
    const serializedNewIncrement = newIncrement.toJSON()

    return serializedNewIncrement;
  }

  async getAllIncrements(sortBy: string, sort: 'asc' | 'desc', productId?: string): Promise<{ increments: Increment[], incrementsCount: number }> {
    const [increments, incrementsCount] = await this.incrementRepository.getAllIncrements(sortBy, sort, productId);
    const serializedIncrements = increments.map(increment => increment.toJSON());
  ...
  async getIncrementById(id: string, eager: boolean): Promise<Increment | null> {
    const increment = await this.incrementRepository.getIncrementById(id, eager)
  ...
  async getLatestIncrement(productId?: string): Promise<Increment | null> {
    const increment = await this.incrementRepository.getLatestIncrement(productId)
    const serializedIncrement = increment!.toJSON()
    return serializedIncrement;
  }
