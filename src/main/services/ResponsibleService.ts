...
export class ResponsibleService {
  ...
  async createResponsible(data: any): Promise<Responsible> {
    ...
    let responsible: Responsible = buildResponsibleEntity(data)
    responsible.productId = productId;

    return await this.responsibleRepository.createResponsible(responsible);
  }

  async getAllResponsibles(productId?: string): Promise<{ responsibles: Responsible[], responsiblesCount: number }> {
    const [responsibles, responsiblesCount] = await this.responsibleRepository.getAllResponsibles(productId);
