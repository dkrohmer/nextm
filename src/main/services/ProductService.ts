...
import { buildProductEntity, buildResponsibleEntity } from '../helpers/entityBuilder';
import { ResponsibleRepository } from '../repositories/ResponsibleRepository';
import { IncrementRepository } from '../repositories/IncrementRepository';
import dataflow from '../../renderer/applets/model-editor/shapes/dataflow';
...
export class ProductService {
  ...
  async getAllProducts(limit: number, offset: number, sort: 'asc' | 'desc', sortby: string): Promise<{ products: Product[], productsCount: number }> {
    const [products, count] = await this.productRepository.getAllProducts(limit, offset, sort, sortby);

    for (const product of products) {
      product.latestIncrementId = await this.incrementRepository.getLatestIncrementId(product.id);
    }
  ...
  async updateProduct(id: string, data: any) {
    ...
    const filteredResponsibles = responsibles.filter((resp: Responsible) => {
        return (resp.firstName || resp.lastName) && { ...resp, role: resp.role || null };
    });
    ...
    const existingResponsibleMap = new Map(existingResponsibles.map((resp: any) => [resp.id, resp]));
    const newResponsibleMap = new Map(filteredResponsibles.map((resp: any) => [resp.id, resp]));
    ...
    for (const existingResponsible of existingResponsibles) {
        if (!newResponsibleMap.has(existingResponsible.id)) {
            await this.responsibleRepository.deleteResponsible(existingResponsible.id);
        }
    }
    ...
    for (const newResponsible of filteredResponsibles) {
        if (existingResponsibleMap.has(newResponsible.id)) {
            ...
            const existingResponsible = existingResponsibleMap.get(newResponsible.id);
            if (existingResponsible!.firstName !== newResponsible.firstName ||
                existingResponsible!.lastName !== newResponsible.lastName ||
                existingResponsible!.role !== newResponsible.role) {
                await this.responsibleRepository.updateResponsible(newResponsible.id, newResponsible);
            }
        } else {
